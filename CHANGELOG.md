...
* **新增“用户标记”功能**：
    * 现在可以在用户的头像下方直接为其添加或编辑自定义标记（备注），方便记忆和识别。
    * 标记信息支持通过“设定同步”中的导出/导入功能进行备份和恢复。
    * UI与现有的用户屏蔽功能和谐共存，并对 S1 NUX 等自定义样式的显示效果进行了兼容性优化。
* **新增“云端自动同步”**：
    * 在“设定同步”标签页中增加了对 Tampermonkey 原生云同步的支持。
    * 开启后，脚本的所有设置（包括屏蔽列表、用户标记、自定义导航等）将在您的多台设备间自动同步。
    * 内置时间戳机制，以后保存的配置为准，智能地避免同步冲突。
* **增强用户屏蔽效果**：
    * 现在当您屏蔽一个用户后，其在帖子中的所有评分记录也会被一并隐藏，实现更彻底的屏蔽体验。

### 🎨 优化与重构

* **重构“关键字屏蔽”编辑器**：
    * 移除了“保存规则”按钮，改为更高效的自动保存机制。对规则的任何修改都会在用户停止输入后自动生效，提升了操作流畅性。
    * 为输入框增加了防抖（Debounce）处理，解决了在快速输入时可能导致的性能问题。
    * 优化了工作流程，新添加的规则将默认处于“不启用”状态，需要用户手动勾选以激活。

### 🐛 问题修复

* 修复了在“关键字屏蔽”设置中，旧版保存逻辑会导致UI组件（如下拉列表）无响应以及添加新规则时输入框重复出现的严重问题。
